...
import { PromptButtons } from '@renderer/features/tasks/conversations/prompt-button';
    ...
    <TooltipProvider>
      <div className="border-t border-border px-2 flex items-center gap-2 h-[41px]">
        <PromptButtons
          ...
          canApply
          tooltipText={(label) => `Add ${label} to the initial message`}
          onApply={(action) => onActionClick(action.text)}
        />
        {issueAction ? (
          <Tooltip>
            <TooltipTrigger>
